...
def get_node_info(item):
    try:
        ...
        # --- 修复重复命名的核心：加入 secret 后4位 ---
        tag = server.split('.')[-1].replace(']', '') if '.' in server else "v6"
        unique_id = str(secret)[-4:] # 取末尾4位确保不重复
        name = f"{p_type.upper()}_{tag}_{port}_{unique_id}_{beijing_time}"
...
def main():
    ...
    for n in raw_nodes:
        ...
        if key not in seen_keys:
            unique_nodes.append(n); seen_keys.add(key)
    ...
    for n in unique_nodes:
        ...
        if n["type"] == "hysteria2":
            ...
            clash_proxies.append({"name": n["name"], "type": "vless", "server": srv_clash, "port": n["port"], "uuid": n["secret"], "tls": True, "udp": True, "servername": n["sni"], "network": "tcp", "reality-opts": {"public-key": n["pbk"], "short-id": n["sid"]}, "client-fingerprint": "chrome"})
    ...
    print(f"✅ 修复完成! 节点名已唯一化。有效节点: {len(clash_proxies)}")
